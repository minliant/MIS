...
class SelFuseFeature(nn.Module):
    '''
    shift_n:steps in article though ablation experiment to certain
    auxseg:the unet original output
    '''
...
class DFUNet(nn.Module):
    '''
    selfeat: True is using DF module
    '''
    def __init__(self, img_ch=3, num_class=1, selfeat=True, shift_n=5, auxseg=True):
